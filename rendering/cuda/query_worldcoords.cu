...
/*
 * occ_idx: 每个 batch 中有多少个 occ
 * coor_2_occ: 对应的 voxel 是否被 occupy，如果是，则为0，不是则为 -1
 * occ_2_coor: occ_idx 中第 i 个 occ 对应的 voxel 索引
 */
__global__ void claim_occ(
    const float* in_data,   // B * N * 3, point_xyz_w_tensor
    const int* in_actual_numpoints, // B, actual_numpoints_tensor
    const int B,
    const int N,
    const float *d_coord_shift,     // 3, ranges
    const float *d_voxel_size,      // 3, scaled_vsize
    const int *d_grid_size,       // 3, scaled_vdim
    ...
    int i_batch = index / N;  // index of batch, cur: N=65536
    if (i_batch >= B) { return; }
    int i_pt = index - N * i_batch;     // i_pt 表示是在当前这个 batch 的点云中的索引
    if (i_pt < in_actual_numpoints[i_batch]) {
        int coor[3];
        const float *p_pt = in_data + index * 3;
        // 坐标减去最小值，除以 voxel 大小，得到 voxel 索引
...
/*
 * occ_idx: 没变
 * coor_occ: 如果在之前某个 coor 已经被 occupy, 那将其周围 query_size 内的 coor 置为 1，包括其自己
 * coor_2_occ: 存储对应 voxel 索引在 occ_idx 对应的 occ 索引
 * occ_2_coor: 存储对应 occ 的 voxel 索引
 */
__global__ void map_coor2occ(
    const int B,
    const int *d_grid_size,       // 3, scaled_vdim
    const int *kernel_size,       // 3, query_size
    const int grid_size_vol,
    const int max_o,
    int *occ_idx, // B, all -1
    ...
    if (i_pt < occ_idx[i_batch] && i_pt < max_o) {
        ...
        int coor_indx_b = i_batch * grid_size_vol + coor[0] * (d_grid_size[1] * d_grid_size[2]) + coor[1] * d_grid_size[2] + coor[2];
        coor_2_occ[coor_indx_b] = i_pt;     // 得到一个 voxel 索引对应于 occ_idx 中的 occ 索引
...
/*
 * occ_2_pnts: 对应索引 occ 处有哪些 points
 * occ_numpnts: 对应索引 occ 处，有多少个 points
 */
__global__ void fill_occ2pnts(
    ...
    if (i_pt < in_actual_numpoints[i_batch]) {
        ...
        if (voxel_idx > 0) {  // found an claimed coor2occ
            ...
            if (tmp < P) {      // 如果数量没有超过设定的最大数量 (memory)，则加点
...
/*
 * raypos_mask: 对应的需要采样的 sample 是否 valid, valid 为 1, 否则为 0
 */
__global__ void mask_raypos(
    float *raypos,    // [B, 2048, 400, 3], raypos_tensor
    int *coor_occ,    // B * 400 * 400 * 400, coor_occ_tensor
    const int B,       // 3
    const int R,       // 3
    const int D,       // 3
    const int grid_size_vol,
...
__global__ void get_shadingloc(
    ...
    int i_batch = index / (R * D);  // index of batch
    if (i_batch >= B) { return; }
    int temp = raypos_mask[index];  // 这里 temp 并不是 0 or 1，而是对应 ray 上的第几个有效点.
    if (temp >= 0) {
...
std::vector<torch::Tensor> woord_query_grid_point_index_cuda(
  ...
  // torch::cuda::synchronize();
  auto ray_mask_tensor = torch::gt(std::get<0>(torch::max(raypos_mask_tensor, -1, false)), 0);  // [B, N]
  ...
  if(R > 0){
        ...
        // volume rendering 中的采样点，有效的前 SR 个被留下（这也是为什么要设置 D 和 SR 两个值）
        // 会首先采 D 个位置，然后取有效的前 SR 个采样点。如果只采 SR 个位置，可能会不足 SR 个，因为会有无效的，即找不到 KNN 的.
  ...
  return {sample_pidx_tensor, sample_loc_tensor, ray_mask_tensor.toType(torch::kInt8), occ_numpnts_tensor, occ_2_pnts_tensor, coor_occ_tensor};
}